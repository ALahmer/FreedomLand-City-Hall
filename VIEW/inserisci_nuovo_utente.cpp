...
InserisciNuovoUtente::InserisciNuovoUtente(QWidget* parent) : QWidget(parent), tipoScelto("Amministratore") {

    setFixedSize(390,280);

    titolo=new QLabel("Inserisci un nuovo utente",this);
    ...
    QStringList tipi;
    tipi<<"Amministratore"<<"Cittadino"<<"Cittadino Straniero"<<"Impiegato Anagrafe"<<"Impiegato Biblioteca";
    ...
    connect(conferma,SIGNAL(clicked()),this,SLOT(InserisciUtente()));
...
void InserisciNuovoUtente::cambiatoTipo(const QString& tipo) {
    if(tipo == "Amministratore")
        tipoScelto = "Amministratore";
    else if(tipo == "Cittadino")
        tipoScelto ="Cittadino";
    else if(tipo == "Cittadino Straniero")
        tipoScelto = "Cittadino Straniero";
    else if(tipo == "Impiegato Anagrafe")
        tipoScelto = "Impiegato Anagrafe";
    else if(tipo == "Impiegato Biblioteca")
        tipoScelto = "Impiegato Biblioteca";
}

void InserisciNuovoUtente::centraFinestra(int windowWidth, int windowHeight) {
...
void InserisciNuovoUtente::InserisciUtente() {
    if (nomeEdit->text().isEmpty() || cognomeEdit->text().isEmpty() || usernameEdit->text().isEmpty() || passwordEdit->text().isEmpty()) {
        ...
        QMessageBox avviso;
        avviso.setText("Vuoi inserire l'utente <i>"+usernameEdit->text()+"</i> ?");
        ...
        if(choice==QMessageBox::Yes)
            emit AddUtente(nomeEdit->text(),cognomeEdit->text(),usernameEdit->text(),passwordEdit->text(),tipoScelto);
    }
}


InserisciNuovoUtente::~InserisciNuovoUtente() {
    delete titolo;
    delete username;
    delete usernameEdit;
    delete nome;
